...
import com.netease.nim.uikit.business.session.NimLocationProvider;
...
public final class NimUIKitImpl {
    ...
    */
    public static AbortableFuture<LoginInfo> login(LoginInfo loginInfo, final RequestCallback<LoginInfo> callback) {
        ...
        loginRequest.setCallback(new RequestCallback<LoginInfo>() {
            ...
            public void onFailed(int code) {
                Log.v("NimPlugin", "login onFailed"+code);
                callback.onFailed(code);
            }

            @Override
            public void onException(Throwable exception) {
                Log.v("NimPlugin", "login onFailed"+exception.getMessage());
    ...
    // 初始化定位
    private static void initLocationProvider(LocationProvider location) {
        if (locationProvider == null) {
            locationProvider = new NimLocationProvider();
        }

        NimUIKitImpl.locationProvider = location;
    }
    ...
    public static void startP2PSessionByAccount(Context context, String account,Map<String,String> userMap) {
        NimUIKitImpl.startChatting(context, account, SessionTypeEnum.P2P, commonP2PSessionCustomization, null,userMap);
    }
    ...
    public static void startTeamSession(Context context, String tid,Map<String,String> userMap) {
        startTeamSession(context, tid, null,userMap);
    }


    public static void startTeamSession(Context context, String tid, IMMessage anchor,Map<String,String> userMap) {
        NimUIKitImpl.startChatting(context, tid, SessionTypeEnum.Team, commonTeamSessionCustomization, anchor,userMap);
    }
    ...
    public static void startChatting(Context context, String id, SessionTypeEnum sessionType, SessionCustomization
            customization, IMMessage anchor,Map<String,String> userMap) {
        if (sessionType == SessionTypeEnum.P2P) {
            P2PMessageActivity.start(context, id, customization, anchor,userMap);
        } else if (sessionType == SessionTypeEnum.Team) {
            TeamMessageActivity.start(context, id, customization, null, anchor,userMap);
        }
    }
