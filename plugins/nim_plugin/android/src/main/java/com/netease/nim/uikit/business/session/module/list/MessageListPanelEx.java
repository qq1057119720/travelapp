...
import com.cgm.view.ReportUserActivity;
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            if (!enableRevokeButton(selectedItem)) {
                longClickItemReport(selectedItem, alertDialog);
            }
            // 4 delete
...
//            longClickItemCancelUpload(selectedItem, alertDialog);
        ...
        private boolean enableRevokeButton(IMMessage selectedItem) {
            ...
            if (selectedItem.getStatus()!=null){
                Log.v("userlogin",selectedItem.getStatus().toString());
            }
            if (!NimUIKitImpl.getMsgRevokeFilter().shouldIgnore(selectedItem)){
        ...
        // 长按菜单项--举报
        private void longClickItemReport(final IMMessage selectedItem, final CustomAlertDialog alertDialog) {
            if (recordOnly) {
                return;
            }
            alertDialog.addItem(container.activity.getString(R.string.whistle_blowing), new CustomAlertDialog.onSeparateItemClickListener() {

                @Override
                public void onClick() {
//                    deleteItem(selectedItem, true);
                    Log.v("userlogin", JSONObject.toJSONString(selectedItem));
                    Intent intent=new Intent(alertDialog.getContext(), ReportUserActivity.class);
                    intent.putExtra("nickname",selectedItem.getFromNick());
                    intent.putExtra("sessionId",selectedItem.getSessionId());
